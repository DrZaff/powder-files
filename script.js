...
function renderItinsView() {
  ...
  // group by duration -> country -> state -> city
  const groups = {};
...
function openResortModal({ mode, resortId = null }) {
  ...
  document.getElementById("resort-form")?.addEventListener("submit", async (e) => {
    ...
    try {
      if (mode === "create") {
        const { error } = await supabase.from("resorts").insert(resortInsertRow(payload));
        if (error) throw error;
      } else {
        const { error } = await supabase.from("resorts").update(resortUpdateRow(payload)).eq("id", resortId);
        if (error) throw error;
      }
...
function openItinModal({ mode, itinId = null }) {
      ...
      <div class="grid-3">
        ...
        <div>
          ...
          <select id="t-resort" class="field-select">
            ...
            ${resorts
              .map((r) => `<option value="${r.id}" ${r.id === initial.resortId ? "selected" : ""}>${escapeHtml(r.name)}</option>`)
              .join("")}
          </select>
        </div>
      </div>
  ...
  document.getElementById("itin-form")?.addEventListener("submit", async (e) => {
    ...
    try {
      if (mode === "create") {
        const { error } = await supabase.from("trips").insert(tripInsertRow(payload));
        if (error) throw error;
      } else {
        const { error } = await supabase.from("trips").update(tripUpdateRow(payload)).eq("id", itinId);
        if (error) throw error;
      }
