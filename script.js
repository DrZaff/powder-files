...
// (UI unchanged; fixes auth wiring + RLS-safe registration)
// =========================================================
...
// IMPORTANT: avoid redeclare crashes if script.js is loaded twice
...
/**
 * IMPORTANT CHANGE:
 * We do NOT insert into editor_requests from the browser anymore.
 * A DB trigger (handle_new_user_editor_request) creates the pending row.
 */
async function registerUser({ email, password, username }) {
  const { data, error } = await supabase.auth.signUp({
    ...
    password,
    options: {
      // store username in user_metadata; trigger reads it
      data: { username },
